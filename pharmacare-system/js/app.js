...
const App = {
    ...
    pharmacyPaybill: '247247', // PharmaCare's simulated paybill number
    otpVerified: false,
    ...
    bindEvents() {
        ...
        document.querySelectorAll('input[name="paymentMethod"]').forEach(radio => {
            radio.addEventListener('change', (e) => {
                if (e.target.value === 'mpesa' || e.target.value === 'card') {
                    this.updatePaymentButtonText(e.target.value);
                }
            });
        });
    ...
    updatePaymentButtonText(method) {
        ...
        if (method === 'mpesa') {
            btn.innerHTML = '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="5" y="2" width="14" height="20" rx="2" ry="2"/><line x1="12" y1="18" x2="12" y2="18"/></svg> Pay with M-Pesa';
        } else if (method === 'card') {
            btn.innerHTML = '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="1" y="4" width="22" height="16" rx="2" ry="2"/><line x1="1" y1="10" x2="23" y2="10"/></svg> Pay with Card';
        } else {
            btn.innerHTML = '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg> Complete Payment';
        }
    },
    ...
    showPaymentModal() {
        ...
        document.getElementById('bankAccountNo').textContent = this.generateAccountNumber();
        document.getElementById('mpesaAccountNo').textContent = this.generateAccountNumber();
    ...
    // Generate account number for payments
    generateAccountNumber() {
        return 'PHARMA' + Math.floor(Math.random() * 100000).toString().padStart(6, '0');
    },
    ...
    selectPaymentMethod(method) {
        ...
        if (method === 'mpesa') {
            this.openModal('mpesaPaymentModal');
        } else if (method === 'card') {
            this.openModal('cardPaymentModal');
        } else if (method === 'bank') {
    ...
    initiateMPesaPayment() {
        console.log('initiateMPesaPayment called');
        ...
        console.log('Phone validation passed, starting STK simulation');

        // Show processing animation
        const modal = document.getElementById('mpesaPaymentModal');
        console.log('M-Pesa modal found:', !!modal);
        
        const paymentInfo = modal ? modal.querySelector('.mpesa-payment-info') : null;
        const processing = document.getElementById('mpesaProcessing');
        
        if (paymentInfo) {
            paymentInfo.classList.add('hidden');
            console.log('Payment info hidden');
        }
        if (processing) {
            processing.classList.remove('hidden');
            console.log('Processing shown');
        }

        // Simulate STK push process
        console.log('Starting STK simulation');
        setTimeout(() => {
            console.log('STK Step 1 - Activating step 1');
            const step1 = document.getElementById('stkStep1');
            console.log('STK Step 1 element:', !!step1);
            if (step1) step1.classList.add('active');
            document.getElementById('mpesaProcessingTitle').textContent = 'STK Push Sent';
            document.getElementById('mpesaProcessingText').textContent = 'Enter your M-Pesa PIN on your phone';
        }, 1500);

        setTimeout(() => {
            console.log('STK Step 2');
            document.getElementById('stkStep2').classList.add('active');
            document.getElementById('mpesaProcessingTitle').textContent = 'Processing Payment';
            document.getElementById('mpesaProcessingText').textContent = 'Confirming transaction...';
        }, 3000);

        setTimeout(() => {
            console.log('STK Step 3');
            document.getElementById('stkStep3').classList.add('active');
            document.getElementById('mpesaProcessingTitle').textContent = 'Payment Complete';
            document.getElementById('mpesaProcessingText').textContent = 'Transaction successful!';
        }, 4500);

        setTimeout(() => {
            console.log('M-Pesa final timeout started');
            try {
                // Show success message - update the existing HTML elements
                const receiptNo = this.generateMpesaReceipt();
                const amount = this.pendingTransaction ? this.pendingTransaction.total.toFixed(2) : '0.00';
                console.log('Showing success, receipt:', receiptNo, 'amount:', amount);
                    
                console.log('mpesaTransactionId:', !!document.getElementById('mpesaTransactionId'));
                console.log('mpesaConfirmPhone:', !!document.getElementById('mpesaConfirmPhone'));
                console.log('mpesaConfirmAmount:', !!document.getElementById('mpesaConfirmAmount'));
                console.log('mpesaProcessing:', !!document.getElementById('mpesaProcessing'));
                console.log('mpesaSuccess:', !!document.getElementById('mpesaSuccess'));
                    
                // Update transaction details
                const txId = document.getElementById('mpesaTransactionId');
                const txPhone = document.getElementById('mpesaConfirmPhone');
                const txAmount = document.getElementById('mpesaConfirmAmount');
                const processing = document.getElementById('mpesaProcessing');
                const success = document.getElementById('mpesaSuccess');
                
                if (txId) txId.textContent = receiptNo;
                if (txPhone) txPhone.textContent = phone;
                if (txAmount) txAmount.textContent = '$' + amount;
                ...
                if (success) {
                    ...
                    console.log('Success section classList:', success.classList.toString());
                }
            } catch (err) {
                console.error('Error in M-Pesa success handler:', err.message, err.stack);
            }
        }, 5000);

        this.showToast('STK push sent to ' + phone, 'info');
    },

    // Complete M-Pesa payment from success screen
    completeMpesaPayment() {
        this.closeModal('mpesaPaymentModal');
        // Reset M-Pesa modal state for next time
        const mpesaModal = document.getElementById('mpesaPaymentModal');
        mpesaModal.querySelector('.mpesa-payment-info').classList.remove('hidden');
        mpesaModal.querySelector('.mpesa-processing').classList.add('hidden');
        mpesaModal.querySelector('.mpesa-success').classList.add('hidden');
        document.getElementById('stkStep1').classList.remove('active');
        document.getElementById('stkStep2').classList.remove('active');
        document.getElementById('stkStep3').classList.remove('active');
        document.getElementById('mpesaPhone').value = '';
        
        this.processPaymentCompletion('mpesa');
    },
    ...
    initiateCardPayment() {
        ...
        if (!cardNumber || cardNumber.length < 16) {
            this.showToast('Please enter valid card number', 'warning');
            return;
        }

        if (!expiry || expiry.length < 5) {
            this.showToast('Please enter valid expiry date (MM/YY)', 'warning');
            return;
        }

        if (!cvv || cvv.length < 3) {
            this.showToast('Please enter valid CVV', 'warning');
            return;
        }

        // Show OTP modal for card verification
        document.getElementById('otpSentTo').textContent = '*** *** *** ' + cardNumber.slice(-4);
        this.openModal('otpModal');
        this.showToast('OTP sent for card verification', 'info');

        // Log demo OTP
        console.log('Demo Card OTP: 5678');
    },
    ...
    verifyCardOTP() {
        ...
        if (!otp || otp.length < 4) {
            this.showToast('Please enter valid OTP', 'warning');
            return;
        }

        // Simulate OTP verification (accept 5678)
        if (otp === '5678') {
            document.getElementById('otpProcessing').classList.remove('hidden');
            document.getElementById('otpForm').classList.add('hidden');

            setTimeout(() => {
                this.closeModal('otpModal');
                document.getElementById('otpProcessing').classList.add('hidden');
                document.getElementById('otpForm').classList.remove('hidden');
                document.getElementById('otpInput').value = '';

                // Show success
                const last4 = document.getElementById('cardNumber').value.slice(-4);
                document.getElementById('cardPaymentModal').querySelector('.card-payment-info').classList.add('hidden');
                const successDiv = document.getElementById('cardPaymentModal').querySelector('.card-success');
                if (successDiv) {
                    successDiv.classList.remove('hidden');
                    successDiv.querySelector('.success-message').innerHTML = `
                        <p><strong>Payment Successful!</strong></p>
                        <p>Card: **** **** **** ${last4}</p>
                        <p>Amount: $${this.pendingTransaction.total.toFixed(2)}</p>
                    `;
                }
    ...
    // Complete card payment from success screen
    completeCardPayment() {
        this.closeModal('cardPaymentModal');
        this.processPaymentCompletion('card');
    },

    // Resend card OTP
    resendCardOTP() {
        this.showToast('New OTP sent', 'info');
        console.log('New Demo Card OTP: 5678');
    },
    ...
    processCashPayment() {
        this.processPaymentCompletion('cash');
    },
    ...
    processBankTransfer() {
        ...
        if (!reference) {
            this.showToast('Please enter transaction reference', 'warning');
            return;
        }

        this.processPaymentCompletion('bank');
    },

    // Process payment completion (common for all methods)
    processPaymentCompletion(paymentMethod) {
        if (!this.pendingTransaction) {
            this.showToast('No pending transaction', 'error');
            return;
        }
        ...
        const transaction = {
            ...
            subtotal: this.pendingTransaction.subtotal,
            tax: this.pendingTransaction.tax,
            discount: this.pendingTransaction.discount,
            total: this.pendingTransaction.total,
            paymentMethod: paymentMethod,
            customerId: this.pendingTransaction.customerId,
            status: 'completed',
            date: new Date().toISOString(),
            // Payment specific details
            paymentDetails: {
                pharmacyPaybill: this.pharmacyPaybill,
                mpesaReceipt: paymentMethod === 'mpesa' ? this.generateMpesaReceipt() : null,
                cardLast4: paymentMethod === 'card' ? document.getElementById('cardNumber')?.value.slice(-4) : null,
                bankReference: paymentMethod === 'bank' ? document.getElementById('bankReference').value : null
            }
        };
        ...
        if (paymentMethod === 'mpesa') {
            this.showToast('M-Pesa payment successful!', 'success');
        } else if (paymentMethod === 'card') {
            this.showToast('Card payment successful!', 'success');
        } else if (paymentMethod === 'bank') {
            this.showToast('Bank transfer confirmed!', 'success');
        } else {
...
// Global function for selecting payment method
function selectPaymentMethod(method) {
    App.selectPaymentMethod(method);
}

// Global function for processing M-Pesa payment
function processMpesaPayment(e) {
    e.preventDefault();
    App.initiateMPesaPayment();
}

// Global function for completing M-Pesa payment
function completeMpesaPayment() {
    App.completeMpesaPayment();
}

// Global function for closing M-Pesa and showing receipt
function closeMpesaAndShowReceipt() {
    App.completeMpesaPayment();
}

// Global function for processing card payment
function processCardPayment(e) {
    e.preventDefault();
    App.initiateCardPayment();
}

// Global function for verifying card OTP
function verifyOtp(e) {
    e.preventDefault();
    App.verifyCardOTP();
}

// Global function for resending card OTP
function resendOtp() {
    App.resendCardOTP();
}

// Global function for closing OTP modal
function closeOtpModal() {
    App.closeOtpModal();
}

// Global function for completing card payment
function completeCardPayment() {
    App.completeCardPayment();
}

// Global function for processing bank transfer
function confirmBankTransfer() {
    App.processBankTransfer();
}

// Global function for updating payment button text
function updatePaymentButtonText(method) {
    App.updatePaymentButtonText(method);
}
...
// Global function for saving medicine
function saveMedicine(e) {
    App.saveMedicine(e);
}

// Global function for saving customer
function saveCustomer(e) {
    App.saveCustomer(e);
}

// Global function for saving supplier
function saveSupplier(e) {
    App.saveSupplier(e);
}

// Global function for editing customer
function editCustomer(id) {
    App.editCustomer(id);
}

// Global function for viewing customer
function viewCustomer(id) {
    App.viewCustomer(id);
}

// Global function for deleting customer
function deleteCustomer(id) {
    App.deleteCustomer(id);
}

// Global function for editing supplier
function editSupplier(id) {
    App.editSupplier(id);
}

// Global function for deleting supplier
...
// Global function for deleting user
function deleteUser(id) {
    App.deleteUser(id);
}

// Global function for filtering inventory
function filterInventory() {
    App.filterInventory();
}

// Global function for updating cart customer
function updateCartCustomer() {
    App.updateCartCustomer();
}

// Global function for filtering customers
function filterCustomers() {
    App.filterCustomers();
}

// Global function for saving user
function saveUser(e) {
    App.saveUser(e);
}

// Global function for updating profile
function updateProfile(e) {
    App.updateProfile(e);
}

// Global function for changing password
